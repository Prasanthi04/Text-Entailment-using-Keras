...
#tokenization
words = []
with open("training_data_part1.txt","r", errors='ignore') as data:
...
for item in itertools.chain(words):
   ...
   for i in itertools.chain(item):
      #print(i)
      filename  = open("word_to_vector1.txt",'a')
      sys.stdout = filename
      print(i, model.wv[i], end ='\n')
      
      
      
glove_vectors_file = "word_to_vector1.txt"
...
file = open('bagofwordsconcat.txt', 'w')   
file.write(concat)
file.close()
